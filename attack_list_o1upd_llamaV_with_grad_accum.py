...
def atanh(x: torch.Tensor) -> torch.Tensor:
    return 0.5 * (torch.log(1 + x + 1e-8) - torch.log(1 - x + 1e-8))
...
def my_haha_preprocessing(image: Image, image_processor, device: torch.device):
    ...
    img = torch.tensor(image_processor(image)["pixel_values"]).to(device)
    return img
...
def image_fit_loss(
    ...
    lower_bound = lower_bound.view(1, 1, 1, 3, 1, 1)
    upper_bound = upper_bound.view(1, 1, 1, 3, 1, 1)
...
def train(
    ...
    original_image = original_image.resize((560, 560))
    ...
    image_mean = torch.tensor(processor.image_processor.image_mean).to(device)
    image_std = torch.tensor(processor.image_processor.image_std).to(device)
    
    x_min = (0 - image_mean) / image_std  # x_0 when image = 0
    x_max = (1 - image_mean) / image_std  # x_0 when image = 1
    ...
    for iteration in tqdm(range(num_iterations)):
        ...
        img_loss = image_fit_loss(x_0, x, x_min, x_max)
        ...
        if restart_num > 0 and (iteration + 1) % restart_num == 0:
            with torch.no_grad():
                ...
                mean = image_mean.view(1, 1, 1, 3, 1, 1).to(device)
                std = image_std.view(1, 1, 1, 3, 1, 1).to(device)
                y = (x + x_0) * std + mean
                y.clamp_(min=0, max=1)
                y = (y * 255).to(torch.uint8).to(x.dtype) / 255.0
                x_new = (y - mean) / std - x_0
                wandb.log({
                    "fix_error_mean": (x_new - x).abs().mean(),
                    "fix_error_std": (x_new - x).abs().std()
                })
                p = atanh(10*x + (x_new - x).clone())
                print(x_new.requires_grad)
