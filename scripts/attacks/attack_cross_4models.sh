...
EXP_NAME="gray_crossattack_phi_llama_llava_qwen_0.5"
IMG_ORIG="gray.png"
PROMPT="list" # "list" for all questions in the list
TARGET_TEXT="sure, here it is"
MODEL_NAME="microsoft/Phi-3.5-vision-instruct,alpindale/Llama-3.2-11B-Vision-Instruct,llava-hf/llava-1.5-7b-hf,Qwen/Qwen2-VL-2B-Instruct" 
...
wandb online
CUDA_VISIBLE_DEVICES=0,1,2,3 python3 "src/crossattack_models.py" \
    --exp_name "$EXP_NAME" \
    --img_orig "$IMG_ORIG" \
    --prompt "$PROMPT" \
    ...
    --epsilon "$ATTACK_NORM" \
    --model_weights 1.0 1.0 1.0 1.0 # 0.2 0.8 1.6
    # --target_text_random
wait