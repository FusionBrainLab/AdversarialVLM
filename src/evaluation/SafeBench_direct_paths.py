...
def main(image_path, output_folder, model_suf, device):
    ...
    # Use direct paths instead of constructing from experiment name
    if not os.path.exists(image_path):
        print("ERROR, image doesn't exist:", image_path)
        return
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Adversarial Model Testing with Direct Paths")
    parser.add_argument("--image_path", type=str, help="Full path to the input image", required=True)
    parser.add_argument("--output_folder", type=str, help="Full path to the output folder", required=True)
    parser.add_argument("--model_suf", type=str, help="What model to use for inferencing the adversarial image", required=True)
    parser.add_argument("--cuda_num", type=int, help="Cuda number", required=True)

    args = parser.parse_args()
    main(args.image_path, args.output_folder, args.model_suf, args.cuda_num) 